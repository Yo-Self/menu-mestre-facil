...
- **Item personalizado no caixa (PDV)**: No Terminal de Vendas, o operador pode adicionar itens avulsos (nome + valor livre), como embalagem de presente ou taxas sazonais, sem cadastrar um produto no cardápio. O item aparece na comanda, no total e no comprovante térmico.
- **Desconto no caixa (PDV)**: Desconto em valor fixo (R$) ou porcentagem (%), aplicado no checkout. A aprovação exige **senha da conta** do gestor ou **PIN do caixa** (4–6 dígitos, configurável em Configurações > PDV e Segurança). O desconto é validado no backend, registrado no pedido e impresso no comprovante (subtotal, desconto e total). **Offline**: desconto não é permitido sem conexão; itens personalizados continuam funcionando offline.
...
- **Atualização Automática do Kanban (híbrida e segura)**: O painel de pedidos usa **Supabase Realtime** (`postgres_changes` em `orders`, filtrado por `restaurant_id`) com JWT autenticado — o servidor só entrega eventos de linhas que o dono pode ler via RLS. Em cada evento, o gestor busca **apenas o pedido alterado** (fetch incremental). Há **fallback de polling a cada 60s** (e refetch ao focar a aba) com query **filtrada** (status ativos + finalizados/cancelados das últimas 24h), evitando rebaixar o histórico completo. O cardápio público e o app iOS **não** usam streaming de pedidos (RPC tokenizado + polling no tracking). A tabela `order_items` não está na publication Realtime; leitura pública de itens foi removida do RLS.
...
- **Pagamentos & Resiliência**: Confirmação de pagamentos online (Stripe/InfinitePay) via webhooks no backend. O painel re-sincroniza pedidos ao carregar, via Realtime e no fallback de polling (60s), processando pagamentos realizados enquanto o gestor estava offline.
