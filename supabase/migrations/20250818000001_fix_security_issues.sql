...
-- Description: Fix Security Issues in Supabase Database
-- This migration addresses security advisor warnings and errors

-- 1. Fix UUID generation consistency - use gen_random_uuid() for all tables
ALTER TABLE public.restaurants ALTER COLUMN id SET DEFAULT gen_random_uuid();
ALTER TABLE public.dishes ALTER COLUMN id SET DEFAULT gen_random_uuid();
ALTER TABLE public.categories ALTER COLUMN id SET DEFAULT gen_random_uuid();
ALTER TABLE public.menus ALTER COLUMN id SET DEFAULT gen_random_uuid();
ALTER TABLE public.profiles ALTER COLUMN id SET DEFAULT gen_random_uuid();

-- 2. Enable RLS on waiter_calls table (currently disabled)
...
-- 5. Fix function security by adding explicit search_path
...
-- 6. Fix import function security
...
-- 7. Add security definer to all functions
COMMENT ON FUNCTION public.update_updated_at_column() IS 'Security: Function runs with definer privileges';
COMMENT ON FUNCTION public.import_restaurant_from_json(jsonb, text) IS 'Security: Function runs with definer privileges and includes user ownership checks';

-- 8. Ensure all tables have proper RLS policies
...
WHERE schemaname = 'public' 
  AND tablename IN ('restaurants', 'categories', 'dishes', 'menus', 'profiles', 'complement_groups', 'complements', 'dish_categories', 'waiter_calls');
